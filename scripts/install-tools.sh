...
#
# Deliberately not a package manager: each tool is one release artifact from
# its own repository at the version in tool-versions.env, so CI and a laptop
# install byte-identical binaries and neither depends on a third party's
# formula being up to date.
#
# Anything already on PATH at the pinned version is left alone — this only
# fills gaps.
set -eu
...
case $(uname -m) in
    arm64) astgrep_asset=app-aarch64-apple-darwin.zip ;;
    *) astgrep_asset=app-x86_64-apple-darwin.zip ;;
esac

# already_installed <name> <expected-version> <version-flag>
already_installed() {
    for candidate in "$TOOLS_BIN/$1" "$(command -v "$1" 2>/dev/null || true)"; do
        [ -n "$candidate" ] && [ -x "$candidate" ] || continue
        found=$("$candidate" "$3" 2>/dev/null | tr -d '\r' | grep -o '[0-9][0-9.]*' | head -1)
        [ "$found" = "$2" ] && return 0
    done
    return 1
}

# fetch_zip <url> <binary-name-inside-zip> <destination-name>
fetch_zip() {
    echo "install-tools: fetching $3"
    ...
    extracted=$(find "$work_dir/unpacked" -type f -name "$2" -perm -u+x | head -1)
    if [ -z "$extracted" ]; then
        echo "install-tools: $2 not found inside $1" >&2
        exit 1
    fi
    mv "$extracted" "$TOOLS_BIN/$3"
    chmod +x "$TOOLS_BIN/$3"
}

if already_installed swiftlint "$FLOODLIGHT_SWIFTLINT_VERSION" version; then
...
if already_installed swiftformat "$FLOODLIGHT_SWIFTFORMAT_VERSION" --version; then
...
if already_installed ast-grep "$FLOODLIGHT_ASTGREP_VERSION" --version; then
...
if already_installed periphery "$FLOODLIGHT_PERIPHERY_VERSION" version; then
