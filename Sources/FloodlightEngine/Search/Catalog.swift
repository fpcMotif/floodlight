...
package enum SearchItemRanking {
    ...
    ///
    /// Equal scores break on title then ID so a query returns the same order
    /// whichever pass produced it — the immediate pass and indexed pass must
    /// agree or results visibly reshuffle as the slower pass lands.
