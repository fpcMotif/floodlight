...
final class CatalogRefreshGuard: Sendable {
    ...
    ///
    /// A walk that took longer than `minimumInterval` would otherwise be
    /// immediately eligible to run again the moment it finished.
    func markRefreshed() {
        state.withLock { $0.lastRefresh = Date.timeIntervalSinceReferenceDate }
    }
}
