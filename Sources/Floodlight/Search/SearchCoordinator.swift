...
extension SearchCoordinator {
    ...
    fileprivate func refreshSelectionSnapshot() {
        ...
        // The blob-free probe: what the board needs to know is that a
        // full-size payload exists, never yet what it contains.
        let hasFullImage = entry.kind == .image
            && clipboardStore.imagePayloadInfo(for: entryID)?.hasPayload == true
