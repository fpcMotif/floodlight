...
/// Consistency tests for the transient-binding SQLite fix and the
/// mirror-only-moves-on-success contract in `ClipboardHistoryStore`.
struct ClipboardHistoryStoreConsistencyTests {
    ...
    @Test func textBindingRoundTripsExactBytesForAdversarialAndExtremeStrings() throws {
        ...
        // A NUL byte terminates a C string on both the bind side (length -1
        // means `strlen`) and the read side (`String(cString:)`), so the one
        // corpus string carrying an interior NUL is excluded here and pinned
        // on its own below rather than silently dropped.
        let strings = (AdversarialCorpus.strings + extras)
            .filter { !$0.contains("\u{0000}") }
    ...
    /// Pins the interior-NUL behaviour, which predates the transient-binding
    /// fix and is symmetric: the mirror holds the original Swift string
    /// verbatim, but a second store reading the row back off disk sees the
    /// text cut at the NUL.
    @Test func textWithInteriorNULIsTruncatedAtThatByteOnReadBack() throws {
        let (dbURL, cleanup) = try makeTemporaryDatabaseURL()
        defer { cleanup() }
    ...
    @Test func clearReturnsFalseAndLeavesMirrorUnchangedWhenTableIsGone() throws {
        ...
        #expect(fixture.store.pinnedEntries.map(\.id) == fixture.pinnedIDsBefore)
        #expect(fixture.store.unpinnedEntries.map(\.id) == fixture.unpinnedIDsBefore)
    }
