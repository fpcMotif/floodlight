...
package enum TestBudget {
    ...
    ///
    /// Measured slowdowns are closer to 1.5×. A scaled *deadline* is free when
    /// it is met, so the margin above that costs a passing test nothing. A
    /// scaled *sleep* or scripted delay is not free — it elapses in full on
    /// every sanitized run, and pays the multiple deliberately, because the
    /// window it holds open has to widen by more than the slowdown it is
    /// racing or the flake it exists to prevent comes back.
